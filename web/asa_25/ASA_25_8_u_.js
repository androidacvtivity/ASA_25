...
webform.afterLoad.asa23 = function () {
    // IMPORTANT:
    // Inițializarea clasificatoarelor (CAEM2 + CFP + CFOJ) NU se face aici,
    // fiindcă afterLoad nu se execută la fiecare AJAX rebuild.
    // Totul este mutat în Drupal.behaviors.* (attach), ca să se reîncarce corect mereu.
};
...
(function ($) {
  Drupal.behaviors.asa23StaticClassifiers = {
    attach: function (context, settings) {
        // CFP / TITLU_R5_C31 (clasificator static)
        var $cfp = $('#TITLU_R5_C31', context);
        if ($cfp.length && typeof set_cfp_to_select === 'function') {
            set_cfp_to_select('TITLU_R5_C31');
        }

        // CFOJ / TITLU_R1_C31 (clasificator static)
        var $cfoj = $('#TITLU_R1_C31', context);
        if ($cfoj.length && typeof set_cfoj_to_select === 'function') {
            set_cfoj_to_select('TITLU_R1_C31');
        }
    }
};

// CAEM2 (dinamic) – inițializare în behaviors, ca să meargă și după AJAX rebuild
Drupal.behaviors.asa23Caem2Init = {
    attach: function (context, settings) {
        if (Drupal.settings.mywebform.preview) {
            return;
        }

        var bases = ['CAP4_R_C32', 'CAP5_R_C37'];

        jQuery.each(bases, function (i, base) {
            // prindem și câmpurile simple (#CAP4_R_C32) și cele indexate (#CAP4_R_C32-1, -2, ...)
            var $sels = jQuery('select#' + base + ', select[id^="' + base + '-"]', context);
            if (!$sels.length) {
                return;
            }

            var saved = (Drupal.settings && Drupal.settings.mywebform && Drupal.settings.mywebform.values)
                ? Drupal.settings.mywebform.values[base]
                : null;
            ...
            $sels.each(function () {
                ...
                // index din id (ex: CAP4_R_C32-3 -> 3)
                var idx = null;
                var m = id.match(new RegExp('^' + base + '-(\d+)$'));
                if (m) {
                    idx = parseInt(m[1], 10);
                }

                // valoarea care trebuie selectată (cod CAEM)
                var valForThis = '';
                var currentVal = $el.val() || '';

                if (idx !== null) {
                    // în settings e de obicei array/obiect cu index de la 0 (idx-1)
                    if (saved && typeof saved === 'object') {
                        if (typeof saved[idx - 1] !== 'undefined') {
                            valForThis = saved[idx - 1];
                        } else if (typeof saved[String(idx - 1)] !== 'undefined') {
                            valForThis = saved[String(idx - 1)];
                        } else {
                            valForThis = currentVal;
                        }
                    } else {
                        valForThis = currentVal;
                    }
                ...
                // încărcăm lista doar dacă e goală (altfel e scump, caem_select e mare)
                if ($el.find('option').length <= 1) {
                    if (typeof set_caem_to_select === 'function') {
                        set_caem_to_select(base, valForThis || null, idx);
                    }
                } else if (valForThis && String($el.val()) !== String(valForThis)) {
                    // dacă lista există deja, doar restabilim selecția
                    $el.val(valForThis).change();
                }
            });
        });
    }
