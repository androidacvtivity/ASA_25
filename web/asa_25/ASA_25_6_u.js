...
/**
 * Helper: find static select by id OR by attribute "field" OR by name.
 * Some environments rename IDs (e.g. edit-submitted-...), but keep field/name.
 */
...
if (typeof valueCfp === "undefined" || valueCfp === null) {
    ...
    obj.trigger('change');
    if (obj.hasClass('select2-hidden-accessible')) { obj.trigger('change.select2'); }
...
(function ($) {
  Drupal.behaviors.asa23StaticClassifiers = {
    attach: function (context, settings) {
      ...
      if (!$cfp.length) {
        $cfp = $("select[field='TITLU_R5_C31'], select[name='TITLU_R5_C31']", context);
      }
      ...
      if (!$cfoj.length) {
        $cfoj = $("select[field='TITLU_R1_C31'], select[name='TITLU_R1_C31']", context);
      }
