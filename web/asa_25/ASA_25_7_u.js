...
webform.afterLoad.asa23 = function () {
    // Static classifiers should be available even in preview (otherwise dropdowns are empty)
    set_cfp_to_select('TITLU_R5_C31');
    set_cfoj_to_select('TITLU_R1_C31');

    // Heavy/dynamic logic only when not preview
    if (!Drupal.settings.mywebform.preview) {
...
/**
 * Helper: find a <select> by id OR by name/field attribute (some Drupal renderers rewrite element IDs).
 */
function asa23_get_static_select(selector) {
    var $el = jQuery('#' + selector);
    if ($el.length) return $el;

    $el = jQuery('select[name="' + selector + '"]');
    if ($el.length) return $el;

    $el = jQuery('select[field="' + selector + '"]');
    if ($el.length) return $el;
...
function set_cfp_to_select(selector, valueCfp) {
    var obj = asa23_get_static_select(selector);
    if (!obj.length) {
        return;
    }
    ...
    jQuery.each(cfp_select, function (key, value) {
        var label = value.description + ", " + value.name;

        // IMPORTANT: set the option VALUE to the full label so that @TITLU_R5_C31 prints with text, not only code
        var optValue = label;
        ...
        // Backward compatible: old saved value might be just the code (e.g. "12")
        if (String(valueCfp) === String(value.description) || String(valueCfp) === String(optValue)) {
    ...
    obj.change();
    // If Select2 is attached, make sure UI refreshes
    if (obj.data('select2')) {
...
// -----------------------
// CFOJ (Forma organizatorico-juridică) - TITLU_R1_C31
// -----------------------
var cfoj_select = [
    { description: '420', name: 'Alte întreprinderi individuale' },
    { description: '430', name: 'Societăţi în nume colectiv' },
    { description: '440', name: 'Societăţi în comandită' },
    { description: '450', name: 'Întreprinderi individuale în agricultură (gospodării ţărăneşti (de fermier))' },
    { description: '500', name: 'Societățile pe acțiuni' },
    { description: '510', name: 'Societăţi pe acţiuni de tip închis' },
    { description: '520', name: 'Societăţi pe acţiuni de tip deschis' },
    { description: '530', name: 'Societăţi cu răspundere limitată' },
    { description: '540', name: 'Cooperative de producţie' },
    { description: '541', name: 'Cooperative de întreprinzător' },
    { description: '550', name: 'Cooperative de consum' },
    { description: '560', name: 'Alte cooperative' },
    { description: '590', name: 'Întreprinderi de stat' },
    { description: '620', name: 'Intreprinderi Municipale' },
    { description: '690', name: 'Alte întreprinderi' },
    { description: '700', name: 'Uniuni de persoane juridice' },
    { description: '871', name: 'Instituţii private' },
    { description: '880', name: 'Instituţii publice' },
    { description: '890', name: 'Asociaţii obşteşti' },
    { description: '899', name: 'Asociaţii religioase' },
    { description: '900', name: 'Partide şi alte organizaţii social-politice' },
    { description: '910', name: 'Fundaţii' },
    { description: '930', name: 'Sindicate' },
    { description: '940', name: 'Patronate' },
    { description: '950', name: 'Birou asociat de avocaţi' },
    { description: '960', name: 'Alte organizaţii necomerciale' },
    { description: '970', name: 'Alte forme de asociaţii' },
    { description: '980', name: 'Birou individual de avocaţi' },
    { description: '990', name: 'Birou notarial' },
    { description: '992', name: 'Activitatea de întreprinzător a persoanei fizice' },
    { description: '993', name: 'Întreprinderi cooperatiste (ale cooperaţiei de consum)' },
    { description: '994', name: 'Filialele întreprinderilor rezidente' },
    { description: '995', name: 'Reprezentanţele întreprinderilor rezidente' },
    { description: '996', name: 'Filialele întreprinderilor nerezidente străine' },
    { description: '997', name: 'Reprezentanţele întreprinderilor nerezidente străine' },
    { description: '998', name: 'Alte subdiviziuni' }
];

function set_cfoj_to_select(selector, valueCfoj) {
    var obj = asa23_get_static_select(selector);
    if (!obj.length) {
        return;
    }

    if (typeof valueCfoj === "undefined" || valueCfoj === null) {
        valueCfoj = (Drupal.settings && Drupal.settings.mywebform && Drupal.settings.mywebform.values &&
            typeof Drupal.settings.mywebform.values[selector] !== "undefined")
            ? Drupal.settings.mywebform.values[selector]
            : '';
    }
    ...
    jQuery.each(cfoj_select, function (key, value) {
        var label = value.description + ", " + value.name;

        // IMPORTANT: set VALUE to full label, so @TITLU_R1_C31 prints with text (not only the code)
        var optValue = label;
        ...
        // Backward compatible: old saved value might be just the code (e.g. "880")
        if (String(valueCfoj) === String(value.description) || String(valueCfoj) === String(optValue)) {
            opt.attr("selected", "selected");
        }
...
/**
 * Ensure static classifiers (CFP/CFOJ) are populated even when fields appear via AJAX / late rendering.
 * This fixes cases where webform.afterLoad runs before #TITLU_R1_C31 exists in DOM.
 */
(function ($) {
  Drupal.behaviors.asa23StaticClassifiers = {
    attach: function (context, settings) {
      var $cfp = $('#TITLU_R5_C31', context);
      if (!$cfp.length) { $cfp = $('select[name="TITLU_R5_C31"]', context); }
      if (!$cfp.length) { $cfp = $('select[field="TITLU_R5_C31"]', context); }
if ($cfp.length && !$cfp.data('asa23-cfp-filled')) {
        $cfp.data('asa23-cfp-filled', 1);
        if (typeof set_cfp_to_select === 'function') {
          set_cfp_to_select('TITLU_R5_C31');
        }
      }

      var $cfoj = $('#TITLU_R1_C31', context);
      if (!$cfoj.length) { $cfoj = $('select[name="TITLU_R1_C31"]', context); }
      if (!$cfoj.length) { $cfoj = $('select[field="TITLU_R1_C31"]', context); }
if ($cfoj.length && !$cfoj.data('asa23-cfoj-filled')) {
        $cfoj.data('asa23-cfoj-filled', 1);
        if (typeof set_cfoj_to_select === 'function') {
          set_cfoj_to_select('TITLU_R1_C31');
        }
      }
    }
...
/**
 * Extra safety: sometimes TITLU_R1_C31 is injected after webform.afterLoad.
 * We retry a few times + after AJAX calls, but only if the select is still empty.
 */
function asa23_fill_static_classifiers() {
    try {
        var $cfp = asa23_get_static_select('TITLU_R5_C31');
        if ($cfp.length && $cfp.find('option').length <= 1 && typeof set_cfp_to_select === 'function') {
            set_cfp_to_select('TITLU_R5_C31');
        }

        var $cfoj = asa23_get_static_select('TITLU_R1_C31');
        if ($cfoj.length && $cfoj.find('option').length <= 1 && typeof set_cfoj_to_select === 'function') {
            set_cfoj_to_select('TITLU_R1_C31');
        }
    } catch (e) {}
}
...
(function ($) {
    // retry shortly after load
    $(function () {
        asa23_fill_static_classifiers();
        setTimeout(asa23_fill_static_classifiers, 200);
        setTimeout(asa23_fill_static_classifiers, 800);
        setTimeout(asa23_fill_static_classifiers, 1500);
    });

    // retry after any AJAX request that might render the field
    $(document).ajaxComplete(function () {
        asa23_fill_static_classifiers();
        setTimeout(asa23_fill_static_classifiers, 0);
    });
})(jQuery);
